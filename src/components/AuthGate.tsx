...
import { getApiBaseUrlForDisplay, topicConfig, type Language } from "@/lib";
import type { AuthContextValue, AuthGateProps } from "./types/AuthGate.types";
import { useProfileQuery, useUpdateProfileLanguageMutation, useAuth } from "@/hooks";
...
export function AuthGate({ children, adminOnly = false }: AuthGateProps) {
  ...
  return (
    <AuthContext.Provider value={{ token, authMode: authMode ?? "guest", username, setPreferredLanguage, linkWithGoogle, linkWithApple, logout, isAdmin }}>
